...
from hamiltonian_path_discovery.prompting.prompt_template import PromptTemplate
from hamiltonian_path_discovery.verification.verification_oracle import VerificationOracle
from hamiltonian_path_discovery.logging.structured_logger import StructuredLogger
from hamiltonian_path_discovery.experimentation.experiment import Experiment
from hamiltonian_path_discovery.pattern_mining.pattern_analyzer import PatternAnalyzer
...
Return ONLY the path as a list of integers, e.g.: [0, 2, 1, 3]""",
    ...
    def execute_prompt(
        ...
        max_energy_threshold: float = 100.0  # Joules
    ) -> PromptResult:
        """
        ...
        Args:
            ...
            max_energy_threshold: Maximum energy usage allowed in Joules (default: 100.0)
        ...
        try:
            ...
            if self.energy_monitor:
                self.energy_monitor.start_monitoring(f"prompt_{template_name}")
            ...
            while current_attempt < max_attempts:
                try:
                    ...
                    current_energy = 0.0
                    if self.energy_monitor:
                        energy_metrics = self.energy_monitor.get_current_metrics()
                        current_energy = energy_metrics.get("total_energy_joules", 0)
                    ...
                    current_result = PromptResult(
                        ...
                        energy_metrics={"total_energy_joules": current_energy},
                    ...
                    if total_energy >= max_energy_threshold:
                        error = "Exceeded energy threshold"
                        current_result = PromptResult(
                            template_name=template_name,
                            input_params=params,
                            generated_code=path,
                            verification_result={"is_valid": False, "error": error},
                            energy_metrics={"total_energy_joules": total_energy},
                            execution_time=time.time() - start_time,
                            success=False,
                            error=error
                        )
    ...
    def _update_metrics(self, template_name: str, result: PromptResult):
        ...
        if result.energy_metrics:
            metrics["average_energy"] = (
                (metrics["average_energy"] * (n-1) + 
                 result.energy_metrics["total_energy_joules"]) / n
                if n > 0 else result.energy_metrics["total_energy_joules"]
            )
    ...
    def get_template_performance(self, template_name: str) -> Dict:
        ...
        avg_energy = np.mean([
            r.energy_metrics.get("total_energy_joules", 0) 
            for r in results
        ])
