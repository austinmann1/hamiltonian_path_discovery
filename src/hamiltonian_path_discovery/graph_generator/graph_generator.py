...
class GraphGenerator:
    ...
    def generate_graph(self, n_vertices: int, density: float = 0.5) -> np.ndarray:
        """
        Generate a random graph with given number of vertices and edge density.
        
        Args:
            n_vertices: Number of vertices in the graph
            density: Probability of edge creation between any two vertices
            
        Returns:
            np.ndarray: Adjacency matrix of the generated graph
        """
        # Generate random adjacency matrix
        adj_matrix = np.random.random((n_vertices, n_vertices)) < density
